...
/>
          ...
          {/* Desktop Navigation */}
          <nav className="hidden lg:flex items-center gap-8 translate-y-4">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-[#C9A44C] hover:text-[#A67C00] transition-colors font-semibold"
              >
                {link.label}
              </Link>
            ))}
...
className="flex items-center gap-2 h-14 px-5 rounded-xl bg-secondary hover:bg-secondary/80 transition-colors translate-y-1"
